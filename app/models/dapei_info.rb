...
class DapeiInfo < ActiveRecord::Base
  ...
  scope :by_user, lambda { |uids|
    joins("INNER JOIN ( dapei_item_infos INNER JOIN matters ON dapei_item_infos.matter_id = matters.id ) ON dapei_infos.id = dapei_item_infos.dapei_info_id ").where("matters.user_id" => uids).order('dapei_infos.created_at desc')
