...
var SJBindex = {
  ...
  goToChannel: function(whichChannel) {
    ...
    lastLink.removeClass('channel-link-current').children('img').attr('src', '/assets/m/icons/' + lastLink.attr('id') + '.png');
    nextLink.addClass('channel-link-current').children('img').attr('src', '/assets/m/icons/' + nextLink.attr('id') + '_current' + '.png');
...
//    $page[0].addEventListener('touchend', touchend, false);
  ...
  setHeader: function(headerInfo) { // headerInfo 是一个dict，包含header中的所需信息。
    ...
    leftIconSrc && $header.children('.header-left-icon').children('img').attr('src', '/assets/m/icons/' + headerInfo.left.icon + '.png').click(headerInfo.left.clickFn);
    rightIconSrc && $header.children('.header-right-icon').children('img').attr('src', '/assets/m/icons/' + headerInfo.right.icon + '.png').click(function() { 
    });
