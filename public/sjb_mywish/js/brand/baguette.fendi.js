	...
	var Product = function(imgNode) {
		...
		var getMetaInfo = function() {
			...
			function _getSingleProduct() {
			    var nameNode = $("div.product-name"),
			    refNode = $("div.product-code"),
			    priceNode = $("span.price:first"),
//			    colorNodes = $("li.option-variant ul.option-value-set img"),
			    sizeNodes = $("div.product-options dd select option");
			    
				if (nameNode.length > 0) {
					result.name = nameNode.text().trim();
					result.name = result.name.replace(/\s{2,}/g, " ");
				}
