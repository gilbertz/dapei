	...
	var Product = function(imgNode) {
		...
		var getMetaInfo = function() {
			...
			function _getSingleProduct() {
				...
				if ($("span.price-amount").length > 0) {
					result.oriPrice = $("span.price-amount").text().trim();
                    if ($("span.price-sale").length > 0) {
                        result.promotionPrice = $("span.price-sale").text().trim();
                    }
				}
				if ($("p.product-id").length > 0) {
					result.refNo = $("p.product-id").first().text().replace(/[a-zA-Z.\u4E00-\u9FA5]/g, '').trim();
				}
				if ($("ul.color-set a").length > 0) {
					var colors = [];
					$("ul.color-set a").each(function() {
                        var colorOption = {};
						colorOption.title = $(this).attr("title");
                        colorOption.src = $(this).find('img').attr('src');
						if ($(this).parent().hasClass('color-selected')) {
